...
### sessionStrorage 与 localStorage 的区别

sessionStorage 把数据保存在 session 对象中。localStorage 将数据保存在客户端本地的硬件设备中，即使浏览器关闭了，数据依然存在。

两者的区别在于， sessionStorage 为临时保存，而 localStorage 为永久保存。

### sessionStorage 和 localStorage 的使用方法，复杂数据存储，JS 对象存储

`storage.setItem(key, value)` 或者 `storage.key = value`

对于复杂的对象，可以使用 JOSON 格式存储，`JSON.stringify(data)` `JSON.parse(str)`
